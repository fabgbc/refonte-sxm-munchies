  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/contact", {
        ...
        body: JSON.stringify({ ...formData, serviceType: "buffet-patisserie", _turnstileToken: turnstileToken }),
      });
      if (response.ok) {
        setIsSuccess(true);
            ...
            >
              ...
              <ul className="space-y-4 max-w-md mx-auto">
                <li className="flex items-center gap-3 text-[var(--color-text-secondary)]">
                  ...
                  Suggested for 10 persons minimum
                </li>
                <li className="flex items-center gap-3 text-[var(--color-text-secondary)]">
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--color-accent)" strokeWidth="2">
                    <polyline points="20 6 9 17 4 12" />
                  </svg>
                  Available for villa events or boat days
                </li>
                <li className="flex items-center gap-3 text-[var(--color-text-secondary)]">
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--color-accent)" strokeWidth="2">
                    <polyline points="20 6 9 17 4 12" />
